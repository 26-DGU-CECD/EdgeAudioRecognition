...
| 모델 | Accuracy | Macro recall | Macro F1 | 평가 샘플 | 간단 평가 |
| --- | ---: | ---: | ---: | ---: | --- |
| `EfficientAT` | 0.635 | 0.93 | 0.692 | 156 | 위험 클래스 recall이 전반적으로 높아 최종 후보로 가장 적합 |
| `YAMNet` | 0.656 | 0.81 | 0.672 | 1,560 | 가볍고 준수하지만 window 기준 결과라 보조 후보 |
| `AST` | 0.494 | 0.33 | 0.258 | 156 | 일부 클래스는 맞추지만 전체 균형이 낮음 |
| `BEATs` | 0.045 | 0.10 | 0.030 | 156 | 현재 설정에서는 학습이 거의 잡히지 않음 |
| `PANNs_CNN14` | 0.038 | 0.11 | 0.008 | 156 | 한 클래스 쏠림이 커서 재학습/설정 수정 필요 |
